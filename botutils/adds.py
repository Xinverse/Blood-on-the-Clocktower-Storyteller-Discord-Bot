...
async def add_alive_role(client, member_obj):
...
async def remove_alive_role(client, member_obj):
    """Remove the alive role to a player"""
    role = client.get_guild(int(SERVER_ID)).get_role(int(ALIVE_ROLE_ID))
    await member_obj.remove_roles(role)


async def add_dead_role(client, member_obj):
    """Grant the dead role to a player"""
    role = client.get_guild(int(SERVER_ID)).get_role(int(DEAD_ROLE_ID))
    await member_obj.add_roles(role)


async def remove_dead_role(client, member_obj):
    """Remove the dead role to a player"""
    role = client.get_guild(int(SERVER_ID)).get_role(int(DEAD_ROLE_ID))
    await member_obj.remove_roles(role)


async def remove_all_alive_roles_pregame(client):
    """Remove the alive roles from all players during pre-game"""
    role = client.get_guild(int(SERVER_ID)).get_role(int(ALIVE_ROLE_ID))
    import main
    for userid in main.master_state.pregame:
        member_obj = client.get_guild(int(SERVER_ID)).get_member(int(userid))
        await member_obj.remove_roles(role)
